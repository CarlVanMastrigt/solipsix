...
struct sol_buddy_tree
{
	...
	uint32_t* availablity_masks_32;
	uint8_t* availablity_masks_8;
	uint32_t total_size_exponent;// essentially a debug only value...
};
...
bool sol_buddy_tree_acquire(struct sol_buddy_tree* tree, uint32_t allocation_size, uint32_t* allocation_offset);
