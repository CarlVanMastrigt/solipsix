...
/** use the top bit of uint32 as the "count" for internal retains and conditions */
#define SOL_BARRIER_INTERNAL_COUNTER_BIT ((uint_fast32_t)0x80000000)

static void sol_sync_barrier_release_references_polymorphic(struct sol_sync_primitive* primitive, uint32_t count);

static void sol_sync_barrier_impose_conditions_polymorphic(struct sol_sync_primitive* primitive, uint32_t count)
{
    /** undo polymorphism */
    struct sol_sync_barrier* barrier = (struct sol_sync_barrier*)primitive;
...
static void sol_sync_barrier_signal_conditions_polymorphic(struct sol_sync_primitive* primitive, uint32_t count)
{
    /** undo polymorphism */
    struct sol_sync_barrier* barrier = (struct sol_sync_barrier*)primitive;
    ...
    if(old_count == count)/** this is the last dependency this barrier was waiting on, put it on list of available barriers and make sure there's a worker thread to satisfy it */
    {
        ...
        /** is important for deterministic barrier pool usage to release the barrier BEFORE actually signalling successors (if it is not still being retained) 
         * this retained reference ensures that the barrier is retained until all its conditions have been signalled (as opposed to just all external retains) 
         * this is so that barrier conforms to the primitive requirement that an unsatisfied condition also retains the prinmitive
         * note that the barrier itself is invalid after this point, but all necessary data for what follows has already been extracted by closing the hopper */
        sol_sync_barrier_release_references_polymorphic(primitive, SOL_BARRIER_INTERNAL_COUNTER_BIT);
...
static void sol_sync_barrier_retain_references_polymorphic(struct sol_sync_primitive* primitive, uint32_t count)
{
    /** undo polymorphism */
    struct sol_sync_barrier* barrier = (struct sol_sync_barrier*)primitive;
...
static void sol_sync_barrier_release_references_polymorphic(struct sol_sync_primitive* primitive, uint32_t count)
{
    ...
    assert(old_count >= count);/** have released more references than were retained */
...
static void sol_sync_barrier_attach_successor_polymorphic(struct sol_sync_primitive* primitive, struct sol_sync_primitive* successor)
{
    ...
    /** barrier must be retained to set up successors (can technically be satisfied illegally if timing lines up, using queue for re-use will make detection better but not infallible) */


    /** if hopper already locked then barrier has had all conditions satisfied/signalled, so no need to even set a condition on the successor */
    if( ! sol_lockfree_hopper_is_closed(&barrier->successor_hopper))
    {
        sol_sync_primitive_impose_conditions(successor, 1);

        /** create a reference to the sucessor and add it to the hopper */
...
{
    .impose_conditions  = &sol_sync_barrier_impose_conditions_polymorphic,
    .signal_conditions  = &sol_sync_barrier_signal_conditions_polymorphic,
    .retain_references  = &sol_sync_barrier_retain_references_polymorphic,
    .release_references = &sol_sync_barrier_release_references_polymorphic,
...
struct sol_sync_barrier_handle sol_sync_barrier_prepare(struct sol_sync_barrier_pool* pool)
{
    ...
    atomic_store_explicit(&barrier->condition_count, SOL_BARRIER_INTERNAL_COUNTER_BIT, memory_order_relaxed);
    /** need to retain a reference until all conditions are met to ensure retain count will not hit zero, ergo one extra reference that will be released after completing the barrier */
    atomic_store_explicit(&barrier->reference_count, SOL_BARRIER_INTERNAL_COUNTER_BIT, memory_order_relaxed);

    return (struct sol_sync_barrier_handle)
    {
        .primitive = (struct sol_sync_primitive*) barrier,
    };
}
...
void sol_sync_barrier_activate(struct sol_sync_barrier_handle barrier)
{
    ...
    assert(barrier.primitive->sync_functions->signal_conditions == &sol_sync_barrier_signal_conditions_polymorphic);
    sol_sync_barrier_signal_conditions_polymorphic(barrier.primitive, SOL_BARRIER_INTERNAL_COUNTER_BIT);
}

void sol_sync_barrier_impose_conditions(struct sol_sync_barrier_handle barrier, uint_fast32_t count)
{
    assert(barrier.primitive->sync_functions->impose_conditions == &sol_sync_barrier_impose_conditions_polymorphic);
    sol_sync_barrier_impose_conditions_polymorphic(barrier.primitive, count);
}

void sol_sync_barrier_signal_conditions(struct sol_sync_barrier_handle barrier, uint_fast32_t count)
{
    assert(barrier.primitive->sync_functions->signal_conditions == &sol_sync_barrier_signal_conditions_polymorphic);
    sol_sync_barrier_signal_conditions_polymorphic(barrier.primitive, count);
}

void sol_sync_barrier_retain_references(struct sol_sync_barrier_handle barrier, uint_fast32_t count)
{
    assert(barrier.primitive->sync_functions->retain_references == &sol_sync_barrier_release_references_polymorphic);
    sol_sync_barrier_release_references_polymorphic(barrier.primitive, count);
}

void sol_sync_barrier_release_references(struct sol_sync_barrier_handle barrier, uint_fast32_t count)
{
    assert(barrier.primitive->sync_functions->release_references == &sol_sync_barrier_release_references_polymorphic);
    sol_sync_barrier_release_references_polymorphic(barrier.primitive, count);
}

void sol_sync_barrier_attach_successor(struct sol_sync_barrier_handle barrier, struct sol_sync_primitive* successor)
{
    assert(barrier.primitive->sync_functions->attach_successor == &sol_sync_barrier_attach_successor_polymorphic);
    sol_sync_barrier_attach_successor_polymorphic(barrier.primitive, successor);
}
