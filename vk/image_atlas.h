...
struct sol_image_atlas_description
{
	...
	/** TODO: change these to `struct sol_buddy_grid_description` */
	uint8_t image_x_dimension_exponent;
	uint8_t image_y_dimension_exponent;
	uint8_t image_array_dimension;

	/** minimum tile size, useful to specify if this wraps a block compressed format, 
	 * actual allocated regions will be a power of 2 multiplied by this (grid_tile_size * 2^n) */
	u16_vec2 grid_tile_size;
};
...
/** begin of range where reads and writes are able to be recorded for this image atlas (i.e. allowing access to its contents) 
 * must be paired with a call to `sol_image_atlas_2_access_range_end` */
void sol_image_atlas_2_access_range_begin(struct sol_image_atlas_2* atlas);

/** must pass in the moment where all reads and writes for this access range are known to have completed
 * must be paired with a call to `sol_image_atlas_2_access_range_begin` */
...
/** create a tile that will be released at the end of the active access range */
enum sol_image_atlas_result sol_image_atlas_2_obtain_transient_entry(struct sol_image_atlas_2* atlas, u16_vec2 size, struct sol_image_atlas_location* entry_location);
