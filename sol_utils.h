...
static inline uint64_t sol_u64_exp_ge   (uint64_t v){ return (v < 2) ? 0  : 64-__builtin_clzl(v-1); }
static inline uint64_t sol_u64_exp_le   (uint64_t v){ return (v < 1) ? 0  : 63-__builtin_clzl(v  ); }
...
static inline uint32_t sol_u32_exp_le   (uint32_t v){ return (v < 1) ? 0  : 31-__builtin_clz(v  ); }
...
#define SOL_SWAP(A,B) { typeof(A) sol_swap_tmp__i = A; A = B; B = sol_swap_tmp__i; }
